...
async def test_confirmar_comando_exitoso(
    ...
    comando_ejecutado = {**comando_ejemplo, "fue_ejecutado": True, "ejecutado_en": "2025-01-01T14:01:00+00:00"}
    mock_supabase.execute.side_effect = [
...
async def test_comando_desde_prediccion_exitoso(
    ...
    mock_supabase.execute.side_effect = [
        MagicMock(data=[prediccion_ejemplo]),              # lookup predicción
        MagicMock(data=[{**comando_ejemplo, "origen": "ml_model"}]),  # insert
        MagicMock(data=[]),                                 # update fue_aplicado
    ]
